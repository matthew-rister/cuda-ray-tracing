...
__global__ void CreateSceneObjects(Intersectable*** objects, int* size) {
	if (blockIdx.x == 0 && threadIdx.x == 0) {
		*objects = new Intersectable*[*size = 4]{
			...
			new Sphere{glm::vec3{-1.f,     0.f, -1.f}, .5f,   new Metal{glm::vec3{.8f}, .3f}},
			new Sphere{glm::vec3{ 1.f,     0.f, -1.f}, .5f,   new Metal{glm::vec3{.8f, .6f, .2f}, 1.f}},
		};
	}
}
