...
__global__ void CreateSceneObjects(
	...
	*objects = new Intersectable*[4 + n * n];
	...
	curand_init(42, 0, 0, &curand_state);
	...
	for (auto half_n = n / 2, i = -half_n; i < half_n; ++i) {
		for (auto j = -half_n; j < half_n; ++j) {
			...
			if (const glm::vec3 center{cx, .2f, cz}; glm::length(center - glm::vec3{4.f, .2f, 0.f}) > .9f) {
				Material* material;
				if (const auto material_probability = curand_uniform(&curand_state); material_probability < .8f) {
				...
				} else if (material_probability < .95f) {
					material = new Metal{
						...
						.5f * curand_uniform(&curand_state)
					};
				} else {
